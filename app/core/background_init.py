...
【小白视角解释】：
这是系统的"后勤部门"，负责在后台悄悄准备资源：
1. 启动时不阻塞用户，后台慢慢加载AI模型
2. 按依赖顺序初始化：先加载模型 → 再初始化数据库 → 最后加载重排序模型
3. 使用事件机制协调各组件启动顺序
4. 就像餐厅开业前，厨师在后台准备食材，不耽误客人进门

【使用的技术】：
- 异步初始化(asyncio.create_task): 非阻塞式后台任务
- 事件协调(asyncio.Event): 组件间依赖管理
- 线程池执行(asyncio.to_thread): CPU密集型任务不阻塞事件循环
- 工厂模式(ChatModelFactory等): 统一模型创建接口
- 懒加载: 按需初始化，节省启动时间
"""
