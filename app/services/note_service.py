...
"""
【模块功能】：智能笔记管理系统，支持AI辅助

【小白视角解释】：
这是系统的"智能笔记本"，功能包括：
1. 创建笔记时，后台自动分析内容生成标签和分类
2. 支持语义搜索：用自然语言找笔记（不只是关键词匹配）
3. 艾宾浩斯记忆曲线：自动安排复习计划，帮助记忆
4. AI写作辅助：续写、扩写、缩写文本
5. 双写机制：同时存MySQL（结构化）和ChromaDB（向量）

【使用的技术】：
- 向量双写: MySQL存储元数据 + ChromaDB存储向量
- 异步标签生成: 后台LLM分析，不阻塞用户操作
- 艾宾浩斯间隔重复算法: [1,2,4,7,15,30]天复习周期
- 语义搜索: ChromaDB向量相似度检索
- JSON字段过滤: 标签多维度筛选
- SSE流式输出: AI写作辅助实时反馈
- Markdown导出: 支持标准格式导出
"""
...
class NoteService:
    ...
    async def assist_stream(self, content: str, action: str):
        ...
        prompt_template = load_write_assistant_prompts()
