...
class AsyncTextSplitter:
    ...
    async def split_text(self, text: str) -> List[str]:
        ...
        if self.embedding_model:
            chunks = await self.optimize_chunks(chunks)
    ...
    async def optimize_chunks(self, chunks: List[str]) -> List[str]:
        """
        ...
        for i in range(1, len(chunks)):
            # 计算当前片段和下一个片段的语义相似度
            similarity = await self.calculate_similarity(current_chunk, chunks[i])

            # 如果相似度高于阈值，合并两个片段
            if similarity > 0.7:   # 阈值可以根据需要调整
                current_chunk += " " + chunks[i]     # 合并片段
            else:
                optimize_chunks.append(current_chunk)
...
if __name__ == '__main__':
    async def main():
        text_spliter = AsyncTextSplitter()
        print(await text_spliter.split_text("hello world"))
