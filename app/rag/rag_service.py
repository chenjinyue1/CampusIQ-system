...
"""
【模块功能】：检索增强生成(RAG)核心服务，实现智能问答

【小白视角解释】：
这是系统的"大脑"，负责回答用户问题。工作流程像图书馆管理员：
1. 用户问问题 → 先生成一个"假设性答案"（HyDE技术）
2. 用这个假设答案去知识库搜索相关文档
3. 找到多个文档后，用重排序模型给它们打分排序
4. 最后让AI根据最相关的文档生成最终回答

【使用的技术】：
- HyDE (Hypothetical Document Embeddings): 假设性文档嵌入技术，提升检索准确率
- 向量数据库(ChromaDB): 存储文档的向量表示，支持语义搜索
- BM25算法: 传统关键词检索，与向量检索结合形成混合检索
- Reranker模型: 交叉编码器重排序，精准匹配查询意图
- LangChain: AI应用编排框架，管理提示词和模型调用
- 异步并发(asyncio.gather): 同时处理多个文档总结，提升响应速度
- 思考过程回调(thinking_callback): 实时推送处理进度给前端
"""
